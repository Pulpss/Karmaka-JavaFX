...
/**
 * La classe Robot représente un joueur automatisé dans un jeu de cartes. Il hérite de la classe Joueur.
 * Il implémente des méthodes spécifiques pour effectuer des choix stratégiques automatisés pendant le jeu.
 * Le robot peut adopter l'une des trois stratégies prédéfinies : AGRESSIF, DEFENSIF, ou FERMIER.
 */
public class Robot extends Joueur {
    ...
    /**
     * Méthode permettant au robot de faire un choix parmi ses cartes disponibles en fonction du message reçu.
     *
     * @param message Le message indiquant le contexte du choix.
     * @param cartes  La liste des cartes parmi lesquelles le robot doit faire son choix.
    ...
    public Carte choix(String message, ArrayList<Carte> cartes) {
        ...
        if (message.contains("choisit une carte de sa main pour la jouer")) {
            switch (strategie) {
                case AGRESSIF:
                	// Choix de la première carte rouge ou de la première carte si aucune carte rouge n'est disponible.
                    c = cartes.stream().filter(carte -> carte.getCouleur() == Couleur.ROUGE).findFirst()
                           .orElse(cartes.get(0));
                    break;
                case DEFENSIF:
                	// Choix de la première carte bleue ou de la première carte si aucune carte bleue n'est disponible.
                    c = cartes.stream().filter(carte -> carte.getCouleur() == Couleur.BLEU).findFirst()
                            .orElse(cartes.get(0));
                    break;
                case FERMIER:
                	// Choix de la première carte verte ou de la première carte si aucune carte verte n'est disponible.
    ...
    /**
     * Méthode permettant au robot de faire un choix parmi une liste de chaînes de caractères en fonction du message reçu.
     * Cette méthode est utilisée pour les choix liés aux utilisations des cartes ou à l'acceptation/refus d'une carte.
     *
     * @param message Le message indiquant le contexte du choix.
    ...
    /**
     * Méthode permettant au robot d'afficher ses cartes en fonction du message reçu.
     *
    ...
    /**
     * Méthode permettant au robot d'afficher un message en utilisant le Router du jeu.
     *
