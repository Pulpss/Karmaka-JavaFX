...
public class Vol extends Carte {
    ...
    public void pouvoir() throws IOException {
        ...
        if (oeuvresAdv.size() > 0) {
            Carte c = Router.getInstance().choix("Choisissez une carte parmis les oeuvres de votre adversaire.", oeuvresAdv.getCartes());
            Partie.getInstance().getJoueur(Partie.getInstance().getTour()).getMain().ajouter(oeuvresAdv.piocher(c));
        } else {
